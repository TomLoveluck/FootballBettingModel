...
class DecayFeatureBuilder(object):
    ...
    def extract_feature_dict(self, event):
        ...
        state = self.state_manager.read_state(event[self.entity_field])
        feature_dict = {}
...
class SampleDecayFeatureBuilder(DecayFeatureBuilder):
...
class TimeDecayFeatureBuilder(DecayFeatureBuilder):
